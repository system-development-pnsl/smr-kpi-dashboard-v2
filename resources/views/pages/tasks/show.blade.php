        ...
        <div class="flex flex-wrap items-start justify-between gap-4">
            <div class="space-y-2 flex-1 min-w-0">
                <div class="flex items-center gap-2 flex-wrap">
                    ...
                    @if($task->is_overdue)
                        <span class="text-[9px] font-semibold px-1.5 py-0.5 rounded bg-status-red-bg text-status-red uppercase">Overdue</span>
                    @endif
                </div>
                <div class="flex items-center gap-2 flex-wrap">
        ...
        <div class="lg:col-span-2 space-y-4">
            ...
            @if($task->description)
                <div class="card">
                    ...
                    <p class="text-[12px] text-brand-black whitespace-pre-wrap leading-relaxed">{{ $task->description }}</p>
                </div>
            @endif
            ...
            <div class="card">
                ...
                <form id="status-form" data-no-ajax method="POST" action="{{ route('tasks.update-status', $task) }}" class="flex flex-wrap gap-2">
                    @csrf @method('PATCH')
                    @foreach($statusConfig as $val => [$label, $dot])
                        <label class="cursor-pointer">
                            <input type="radio" name="status" value="{{ $val }}" class="sr-only"
                                   {{ $task->status === $val ? 'checked' : '' }}
                                   onchange="this.closest('form').submit()">
                            <div class="flex items-center gap-1.5 px-3 py-2 rounded-lg border text-[11px] font-medium transition-all
            ...
            <div class="card overflow-hidden p-0">
                <div class="flex items-center justify-between px-4 py-3 border-b border-brand-border">
                    <p class="text-[12px] font-semibold text-brand-black">
                        Sub-tasks
                        <span id="subtask-count" class="text-[11px] font-normal text-brand-muted ml-1">({{ $task->subtasks->count() }})</span>
                    </p>
                ...
                <div id="subtask-form" class="px-4 py-3 border-b border-brand-border bg-brand-bg" style="display:none">
                    <form id="subtask-inner-form" method="POST" action="{{ route('tasks.subtask', $task) }}" class="flex flex-wrap gap-2 items-end">
                        @csrf
                        <div class="flex flex-col gap-1 flex-1 min-w-[160px]">
                            <label class="text-[10px] text-brand-muted font-medium uppercase tracking-wide">Title</label>
                            <input type="text" name="title" required maxlength="200" class="input h-8 text-[12px]" placeholder="Sub-task title…">
                        </div>
                        <div class="flex flex-col gap-1">
                            <label class="text-[10px] text-brand-muted font-medium uppercase tracking-wide">Due Date</label>
                        ...
                        <div class="flex gap-2">
                            ...
                            <button type="button" id="btn-cancel-subtask" class="btn-secondary h-8 text-[11px]">Cancel</button>
                        </div>
                    </form>
                </div>
                ...
                @forelse($task->subtasks as $sub)
                    ...
                    <div class="data-row flex items-center gap-3">
                        ...
                        <span class="text-[11px] text-brand-muted flex-shrink-0">{{ $sub->due_date->format('d M Y') }}</span>
                    </div>
                @empty
                    <p id="subtasks-empty" class="text-center py-6 text-[11px] text-brand-subtle">No sub-tasks yet.</p>
                @endforelse
                </div>
            </div>
            ...
            <div class="card overflow-hidden p-0">
                <div class="px-4 py-3 border-b border-brand-border">
                    <p class="text-[12px] font-semibold text-brand-black">
                        Comments
                        <span id="comment-count" class="text-[11px] font-normal text-brand-muted ml-1">({{ $task->comments->count() }})</span>
                    </p>
                </div>
                ...
                @forelse($task->comments->sortByDesc('created_at') as $comment)
                    <div class="px-4 py-3 border-b border-brand-border/60">
                        <div class="flex items-center gap-2 mb-1">
                            <div class="w-5 h-5 rounded-full bg-brand-bg border border-brand-border flex items-center justify-center text-[8px] font-semibold text-brand-muted">
                                {{ strtoupper(substr($comment->user?->full_name ?? '?', 0, 1)) }}
                            </div>
                            <span class="text-[11px] font-medium text-brand-black">{{ $comment->user?->full_name ?? '—' }}</span>
                            <span class="text-[10px] text-brand-subtle">{{ $comment->created_at->format('d M Y H:i') }}</span>
                        </div>
                        <p class="text-[12px] text-brand-black pl-7 whitespace-pre-wrap">{{ $comment->body }}</p>
                    </div>
                @empty
                    <p id="comments-empty" class="text-center py-6 text-[11px] text-brand-subtle">No comments yet.</p>
                @endforelse
                </div>

                {{-- Add comment --}}
                <div class="px-4 py-3 bg-brand-bg border-t border-brand-border">
                    <form id="comment-form" method="POST" action="{{ route('tasks.comment', $task) }}" class="space-y-2">
                        @csrf
                        <textarea name="body" rows="2" required maxlength="2000"
                                  class="textarea text-[12px]" placeholder="Write a comment…"></textarea>
            ...
            <div class="card space-y-3">
                ...
                <div class="space-y-2">
                    ...
                    <div class="flex items-start justify-between gap-2">
                        ...
                        <span class="text-[11px] font-medium text-brand-black">{{ $task->department->label ?? '—' }}</span>
                    </div>
                    @if($task->start_date)
                        <div class="flex items-start justify-between gap-2">
                    ...
                    <div class="flex items-start justify-between gap-2">
                        ...
                        <span class="text-[11px] font-medium {{ $task->is_overdue ? 'text-status-red' : 'text-brand-black' }}">
                    ...
                    @if($task->completed_at)
                        <div class="flex items-start justify-between gap-2">
                            ...
                            <span class="text-[11px] text-status-green">{{ $task->completed_at->format('d M Y') }}</span>
                        </div>
                    @endif
                    <div class="flex items-start justify-between gap-2">
            ...
            <div class="card space-y-2">
                ...
                @forelse($task->assignees as $assignee)
                    <div class="flex items-center gap-2">
                        <div class="w-6 h-6 rounded-full bg-brand-bg border border-brand-border flex items-center justify-center text-[9px] font-semibold text-brand-muted flex-shrink-0">
                ...
                <div class="card space-y-2">
                    ...
                    <div class="flex flex-wrap gap-1.5">
                        @foreach($task->tags as $tag)
                            <span class="text-[10px] px-2 py-0.5 rounded bg-brand-bg border border-brand-border text-brand-muted">{{ $tag }}</span>
                        @endforeach
                    </div>
                </div>
            @endif
                    ...
                    <div class="divide-y divide-brand-border/60 max-h-64 overflow-y-auto">
                        @foreach($task->auditLogs->sortByDesc('created_at') as $log)
                            <div class="px-4 py-2.5">
                                <p class="text-[11px] text-brand-black">{{ $log->action ?? $log->event ?? 'Updated' }}</p>
                                <div class="flex items-center gap-1 mt-0.5">
                                    <span class="text-[10px] text-brand-subtle">{{ $log->user?->full_name ?? 'System' }}</span>
                                    <span class="text-[10px] text-brand-subtle">·</span>
                                    <span class="text-[10px] text-brand-subtle">{{ $log->created_at->format('d M H:i') }}</span>
                                </div>
                            </div>
                        @endforeach
                    </div>
...
document.getElementById('status-form').addEventListener('submit', async function (e) {
    ...
    try {
        ...
        if (res.ok) {
            Toast.fire({ icon: 'success', title: data.message || 'Status updated.' });
            form.querySelectorAll('label').forEach(label => {
                const radio = label.querySelector('input[type="radio"]');
                const div   = label.querySelector('div');
                if (radio.checked) {
                    div.className = div.className
                        .replace('border-brand-border text-brand-muted hover:border-brand-black/30', '')
