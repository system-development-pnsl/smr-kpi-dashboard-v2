...
<div class="flex h-screen w-screen overflow-hidden bg-brand-bg">
                ...
                @foreach($navItems as $item)
                    <li>
                        <a href="{{ route($item['route']) }}"
                           ...
                           data-sidebar-link>
                            @include('components.icons.'.$item['icon'], ['class' => 'w-[15px] h-[15px] flex-shrink-0'])
                            <span class="sidebar-expanded truncate">{{ $item['label'] }}</span>
                            @if(request()->routeIs($item['route'].'*'))
                                <svg class="sidebar-expanded ml-auto w-3 h-3 opacity-60" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        <div class="border-t border-white/10 flex-shrink-0">
                ...
                <a href="{{ route('notifications.index') }}"
                   ...
                   class="sidebar-link relative h-9 px-3 w-full">
                    ...
                    @if(auth()->user()->unreadNotifications->count() > 0)
                        <span class="absolute top-1.5 right-1.5 w-3.5 h-3.5 bg-status-red rounded-full text-[8px] font-bold flex items-center justify-center text-white">
            ...
            <div class="sidebar-expanded px-3 py-3 flex items-center gap-2.5">
                ...
                <div class="flex-1 min-w-0">
                    ...
                    <p class="text-[10px] text-white/40 truncate capitalize">{{ str_replace('_', ' ', auth()->user()->role) }}</p>
                </div>
    ...
    <div class="flex flex-col flex-1 min-w-0 overflow-hidden">
        ...
        {{-- Top Header --}}
        <header class="h-14 bg-brand-surface border-b border-brand-border flex items-center px-4 gap-4 flex-shrink-0">
            {{-- Toggle --}}
            <button id="sidebar-toggle" class="text-brand-muted hover:text-brand-black transition-colors p-1 rounded hover:bg-brand-bg">
...
document.addEventListener('submit', async function (e) {
    ...
    btns.forEach(b => { b.disabled = true; b.innerHTML = '<span style="opacity:.5">…</span>'; });
