...
<div id="{{ $uid }}" data-ss-wrap data-placeholder="{{ $placeholder }}"
    @if ($formId) data-form-id="{{ $formId }}" @endif class="relative {{ $class }}">
    ...
    <button type="button" data-ss-trigger
        ...
        <svg data-ss-chevron class="w-3.5 h-3.5 flex-shrink-0 text-brand-muted transition-transform duration-150"
    ...
    <div data-ss-dropdown
        ...
        <div class="p-2 border-b border-brand-border">
            <div class="flex items-center gap-2 bg-brand-bg border border-brand-border rounded px-2.5 h-7">
                <svg class="w-3 h-3 text-brand-subtle flex-shrink-0" fill="none" stroke="currentColor"
                    viewBox="0 0 24 24">
        ...
        <ul class="max-h-48 overflow-y-auto py-1">
            ...
            @foreach ($options as $opt)
                ...
                <li>
                    <button type="button" data-ss-option data-value="{{ $opt['value'] }}"
                        ...
                        {{ $opt['label'] }}
                        <svg data-ss-check class="inline-block w-3 h-3 ml-1 text-brand-black" fill="none"
