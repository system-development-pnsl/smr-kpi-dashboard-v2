    ...
    <div class="card">
        ...
        <form method="POST" action="{{ route('documents.upload') }}" enctype="multipart/form-data"
            ...
            <div class="flex flex-col gap-1">
                ...
                <x-select-search
                    name="department_id"
                    :options="$departments->map(fn($d) => ['value' => $d->id, 'label' => $d->label])->all()"
            ...
            <div class="flex flex-col gap-1 flex-1 min-w-[180px]">
                <label class="text-[10px] text-brand-muted font-medium uppercase tracking-wide">Description (optional)</label>
        ...
        @error('file')<p class="text-[11px] text-status-red mt-2">{{ $message }}</p>@enderror
        @error('department_id')<p class="text-[11px] text-status-red mt-2">{{ $message }}</p>@enderror
    </div>
        ...
        @forelse($documents as $doc)
            ...
            <div class="data-row group grid grid-cols-12 gap-3">
                ...
                <div class="col-span-4 flex items-center gap-2 min-w-0">
                    <div class="w-7 h-7 rounded bg-brand-bg border border-brand-border flex items-center justify-center flex-shrink-0">
                ...
                <div class="col-span-1 flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                    ...
                    <form method="POST" action="{{ route('documents.destroy', $doc) }}" data-remove-closest=".data-row">
                        @csrf @method('DELETE')
                        <button type="submit" data-confirm="This document will be permanently deleted." class="p-1 text-brand-muted hover:text-status-red transition-colors" title="Delete">
                            <svg class="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
